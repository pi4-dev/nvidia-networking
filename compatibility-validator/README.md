...
**Connection and port map:** in Project / BOM, choose **Build connection map**. Click a device to inspect its physical cage inventory or click a cable/line for its PN, endpoint modes, branch paths, installation states and complete validation checks. A breakout appears as one assembly with a single shared head cage and separate branches. Status colors distinguish compatible, conditional, unknown and incompatible connections; allocation conflicts are highlighted in red even if individual link checks pass.

Filter by device, fabric, result or device/rack/cable/PN text. Use zoom and scroll to navigate; Enter or Space activates focused diagram elements. Large maps display 40 assemblies per page and cage grids display 64 ordinals per page. Counters always describe the complete project, including when the diagram is filtered.

Device details separate physical cages from logical interfaces within the selected mode. For example, one occupied `2x400` head cage contains two logical interfaces and uses one physical slot. Free cages mean unallocated **in this project**. Unknown counts, unresolved profiles and conflicting device identities never generate assumed free capacity. Duplicate and out-of-range cage assignments remain visible. The map is derived from project JSON; editing the draft or changing the catalog invalidates the old view.
...
`app/models.py` and `topology_models.py` own the strict Pydantic contracts; `catalog.py` handles loading and cross-file integrity, `rules.py` single-link compatibility, `hardware.py` scoped evidence/qualification, `recommendations.py` connection selection, `topology.py` complete breakouts, `projects.py` project/BOM/CSV operations, `project_map.py` topology and cage inventory, `cabling.py` installation rows/labels, `cabling_exports.py` PDF/XLSX generation, and `api.py` HTTP routes. `main.py` remains the Uvicorn entry point. Browser helpers and UI logic live in `static/core.js`, `static/app.js`, `static/topology.js` `static/cabling.js` and `static/project-map.js`.
...
| `POST /api/project/map` | Revalidated devices, assemblies, physical terminations, cage capacity and logical interfaces. |
