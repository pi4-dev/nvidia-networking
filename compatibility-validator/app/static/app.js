...
(() => {
  ...
  const state = { meta: null, devices: [], products: [], rows: [], selected: null, linkResult: null, view: 'port', online: false };
  const portGate = new RequestGate(), catalogGate = new RequestGate(), linkGate = new RequestGate();
  ...
  function groupFor(deviceId, groupId) { return state.devices.find(d => d.id === deviceId)?.port_groups.find(g => g.id === groupId); }
  function productFor(id) { return state.products.find(p => p.id === id); }
  function portKey() { return JSON.stringify([state.meta?.revision, $('device').value, $('group').value, $('mode').value, $('fabricFilter').value]); }
  function updateExport() { $('export').disabled = state.view === 'link' ? !state.linkResult : !state.rows.some(i => i.id === state.selected); }
  ...
  function fillPortDevices(wanted = {}) {
    ...
    fillGroups(wanted.group, wanted.mode);
  }
  ...
  function fillModes(id, group, preferred) {
    choices(id, (group?.modes || []).map(m => [m.id, `${m.links} × ${m.speed_gbps}G`]), preferred, group?.modes.length ? 'Any documented mode' : 'Mode unknown');
  }
  ...
  function renderGroup() {
    const g = groupFor($('device').value, $('group').value);
  ...
  function selectProduct(id, pn = null) {
    ...
    $('validationText').textContent = `${item.model}: ${v.status}. ${end ? `Use termination ${end.id} (${end.interface_type}, ${end.role}).` : 'Endpoint details are missing.'} Scope: selected host port.`;
  ...
  async function refreshPort(preferred = state.selected, pn = $('partNumber').value) {
    ...
    state.rows = []; state.selected = null; resetDecision(); renderGroup();
    ...
    try {
      const result = await jsonRequest(`/api/evaluate?${query}`, ticket);
  ...
  function fillLinkGroups(side, wanted = {}) {
    const d = state.devices.find(d => d.id === $(side + 'Device').value);
    choices(side + 'Group', (d?.port_groups || []).map(g => [g.id, g.label]), wanted[side + 'Group']);
    fillModes(side + 'Mode', groupFor($(side + 'Device').value, $(side + 'Group').value), wanted[side + 'Mode']);
  }
  ...
  function connectionRequest() {
    const selection = side => ({ device_id: $(side + 'Device').value, port_group_id: $(side + 'Group').value,
    ...
    return { a: selection('a'), b: selection('b'), fabric: $('linkFabric').value,
      ...
      fiber: optical ? { medium: $('fiberType').value === 'OS2' ? 'SM' : 'MM', fiber_type: $('fiberType').value,
  ...
  function setView(view) {
    state.view = view === 'link' ? 'link' : 'port';
    for (const name of ['port', 'link']) {
  ...
  async function loadCatalog(initial = false) {
    ...
    portGate.cancel(); invalidateLink();
    try {
      ...
      refreshFilters(wanted); fillPortDevices(wanted); fillLink(wanted);
  ...
  function exportedReport() {
    ...
    return { format: 'nvidia-compatibility-report-v1', exported_at: new Date().toISOString(),
      ...
      scope: state.view === 'link' ? 'single-link' : 'host-port',
      result: state.view === 'link' ? state.linkResult : item,
      port_profile: state.view === 'port' ? groupFor($('device').value, $('group').value) : undefined };
  }
  $('kind').addEventListener('change', () => { fillDeviceOptions(); fillGroups(); refreshPort(null); });
  $('device').addEventListener('change', () => { fillGroups(); refreshPort(null); });
  $('group').addEventListener('change', () => { fillModes('mode', groupFor($('device').value, $('group').value)); refreshPort(null); });
  ...
  for (const side of ['a', 'b']) {
    $(side + 'Device').addEventListener('change', () => { $('pinout').checked = false; fillLinkGroups(side); invalidateLink(); });
    $(side + 'Group').addEventListener('change', () => { fillModes(side + 'Mode', groupFor($(side + 'Device').value, $(side + 'Group').value)); invalidateLink(); });
    ...
    for (const field of ['Mode', 'End']) $(side + field).addEventListener('change', invalidateLink);
  ...
  for (const id of ['linkFabric', 'linkLength', 'fiberType', 'connectorA', 'connectorB', 'pinout']) $(id).addEventListener(id === 'linkLength' ? 'input' : 'change', () => {
    if (['fiberType', 'connectorA', 'connectorB'].includes(id)) $('pinout').checked = false;
    invalidateLink();
  });
  ...
  window.addEventListener('pagehide', () => { clearTimeout(pollTimer); portGate.cancel(); catalogGate.cancel(); linkGate.cancel(); });
